...
public class LoginPageScripts {
    public static int returnLoginHistory(String userID) throws SQLException {
        DBQueryExecutor dbQueryExecutor = new DBQueryExecutor();
        Connection conn = dbQueryExecutor.connectBD();
        Statement stmt = conn.createStatement();
        String sql = "select user_id, login_date from tbl_user_login_history where user_id = "+userID+" and login_date >= "+"'"+ CommonFun.getCurrentDate("dd-MMM-yy")+"'";
